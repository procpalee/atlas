 ...
 *  onApply를 주면 updateTask 대신 콜백으로 patch 전달(일괄 작업용) — task 없이도 동작. */
export default function PlanPopover({ task, onClose, align = 'right', vertical = 'down', onApply }: {
  task?: Task
  onClose: () => void
  align?: 'right' | 'left'
  /** 'up'이면 부모 위로 펼침 (하단 고정 바 등) */
  vertical?: 'down' | 'up'
  onApply?: (patch: Partial<Task>) => void
}) {
  const updateTask = useStore(s => s.updateTask)
  const doApply = (patch: Partial<Task>) => {
    if (onApply) onApply(patch)
    else if (task) updateTask(task.id, patch)
  }
  const apply = (patch: Partial<Task>) => { doApply(patch); onClose() }
  ...
  const opts: { icon: React.ReactNode; label: string; sub?: string; active: boolean; onClick: () => void }[] = [
    ...
    { icon: <CircleSlash size={14} className="text-zinc-400" />, label: '날짜 없음', active: !!task && !task.scheduled_date && !task.someday, onClick: () => apply({ scheduled_date: null, someday: false }) },
  ]

  return (
    <>
      <div className="fixed inset-0 z-40" onMouseDown={onClose} />
      <div className={`absolute z-50 w-[208px] max-w-[calc(100vw-1.5rem)] rounded-lg border border-zinc-200 bg-white p-1 shadow-xl dark:border-zinc-700 dark:bg-zinc-900 ${vertical === 'up' ? 'bottom-7' : 'top-7'} ${align === 'right' ? 'right-0' : 'left-0'}`}>
        ...
        <label className="flex items-center gap-2 px-2 py-1 text-[12px] text-zinc-500 dark:text-zinc-400" title="마감일">
          ...
          <input
            ...
            className="w-full bg-transparent text-[12.5px] outline-none"
            value={task?.deadline ?? ''}
            onChange={e => doApply({ deadline: e.target.value || null })}
          />
        </label>
      </div>
    </>
